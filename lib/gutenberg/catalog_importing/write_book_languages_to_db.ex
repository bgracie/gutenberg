defmodule Gutenberg.CatalogImporting.WriteBookLanguagesToDb do
  ...
  defp build_book_languages(book_from_json, books_from_db, languages_from_db) do
    ...
    |> Map.fetch!("language")
    |> Enum.map(
      &build_book_language(&1, book_from_json, books_from_db, languages_from_db)
    )
  end

  defp build_book_language(
         language_code,
         book_from_json,
         books_from_db,
         languages_from_db
    ...
    Map.merge(
      %{book_id: book.id, language_id: language.id},
      Repo.now_timestamps()
    )
  end
end
